...
Example: `config.caladan.toml` adds `~/Downloads` → iCloud Drive Downloads to `[dotfiles]`,
...
`gh/hosts.yml`, `gh/config.yml`, `claude/settings.json`, `obsidian/`, `cmux/`, `docs/TASKS.md`.
...
One clause replaces both of the builtin's:
its own glob, and a detected-type selector that would pull zsh into the tools that cannot parse it.
That type selector is what would otherwise match our extensionless scripts
(`bin/`, mise tasks/hooks) by their shebang,
so dropping it is what obliges the glob to name them.
